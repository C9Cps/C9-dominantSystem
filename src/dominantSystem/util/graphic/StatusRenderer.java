...
import dominantSystem.util.func.MathUtil;
...
public class StatusRenderer {
    ...
    public void registerStatusEffects() {
        register(StatusEffects.burning, 80, (warmup, unit, status) -> {
            Color from = dsPal.thiriumAmmoFront, to = dsPal.thiriumAmmoBack;
        ...
        register(StatusEffects.invincible, 60, (warmup, unit, status) -> {
            int step = 36;
                    ...
                    for (int k = 0; k < 6; k++) {
                        ...
                        if (!(Tmp.v2.x - rad > right() || Tmp.v2.x + rad < left() || Tmp.v2.y - dst > top() || Tmp.v2.y + dst < bottom())) {
                            ...
                            if (rand.chance(chance)) {
                                ...
                                Tmp.c1.set(dsItems.thirium.color).lerp(dsPal.thiriumAmmoBack, innerWarmup * MathUtil.timeValue(0.2f, 0.6f, 1.5f, rand.random(360)));
                                Draw.color(Tmp.c1);
        ...
        register(StatusEffects.none, 100, (warmup, unit, status) -> {
            Color from = Pal.heal, to = dsPal.thiriumAmmoBack;
