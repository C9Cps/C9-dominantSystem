package dominantSystem.util.ui.dialog;
...
import dominantSystem.util.Tool_Internal;
import dominantSystem.util.func.dsInterp;
import dominantSystem.util.func.dsPixmap;

import java.lang.reflect.Field;
import java.util.concurrent.atomic.AtomicInteger;

import static mindustry.Vars.*;
import static dominantSystem.util.ui.TableFunc.LEN;
import static dominantSystem.util.ui.TableFunc.OFFSET;
        ...
        cont.pane(t -> {
                    ...
                    for (Field f : fields) {
                        try {
                            if (f.getType().getSimpleName().equals("TextButtonStyle")) {
                                ...
                                t.table(inner -> {
                                    try {
                                        inner.button(f.getName(), (TextButton.TextButtonStyle) f.get(null), () -> {
                                        }).size(LEN * 3, LEN).row();
                                        inner.button(f.getName(), (TextButton.TextButtonStyle) f.get(null), () -> {
                                        }).size(LEN * 3, LEN).pad(OFFSET / 3).disabled(b -> true).row();
                                    } catch (IllegalAccessException err) {
                                        throw new IllegalArgumentException(err);
                                    }
                                }).grow().pad(OFFSET / 3);
                        ...
                        try {
                            if (f.getType().getSimpleName().equals("ImageButtonStyle")) {
                                ...
                                t.table(inner -> {
                                    inner.table(de -> {
                                        try {
                                            de.button(Icon.none, (ImageButton.ImageButtonStyle) f.get(null), () -> {
                                            }).size(LEN * 3, LEN).row();
                                        } catch (IllegalAccessException err) {
                                            throw new IllegalArgumentException(err);
                                        }
                                    ...
                                    inner.table(de -> {
                                        try {
                                            de.button(Icon.none, (ImageButton.ImageButtonStyle) f.get(null), () -> {
                                            }).disabled(b -> true).size(LEN * 3, LEN).row();
                                        } catch (IllegalAccessException err) {
                                            throw new IllegalArgumentException(err);
                                        }
                                        de.add(f.getName()).pad(OFFSET / 3);
            ...
            t.button("Interps", () -> {
                ...
                dialog.cont.pane(t1 -> {
                    ...
                    fields.addAll(dsInterp.class.getFields());
                    int i = 0;
            ...
            t.button("Sounds", () -> new BaseDialog("ICONS") {
                {
                    ...
                    Seq<Field> fields = Seq.with(Sounds.class.getFields()).addAll(dsSounds.class.getFields());
            ...
            t.button("Generate Icon", () -> ui.loadAnd("[accent]Generating", () -> {
                content.units().each(u -> {
                    if (u.name.contains(dominantSystemMod.MOD_NAME)) {
                        dsPixmap.saveUnitPixmap(Core.atlas.getPixmap(u.fullIcon).crop(), u);
                    }
                });

                dsPixmap.saveAddProcessed();
            })).disabled(b -> !dsPixmap.isDebugging());
