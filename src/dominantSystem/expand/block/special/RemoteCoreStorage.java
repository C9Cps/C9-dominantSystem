package dominantSystem.expand.block.special;
...
import dominantSystem.dsGroups;
...
public class RemoteCoreStorage extends StorageBlock {
    ...
    public RemoteCoreStorage(String name) {
        ...
        consumePowerDynamic((RemoteCoreStorageBuild b) -> dsGroups.placedRemoteCore[b.team.id].size * powerConsumption);
    }

    public static void clear() {
        for (ObjectSet<RemoteCoreStorageBuild> set : dsGroups.placedRemoteCore) {
            set.clear();
        }
    }

    public void drawPlace(int x, int y, int rotation, boolean valid) {
        if (maxPlaceNum(Vars.player.team()) <= dsGroups.placedRemoteCore[Vars.player.team().id].size) {
            drawPlaceText(Core.bundle.get("nh.reach-limit"), x, y, false);
        }
    }
    ...
    public int maxPlaceNum(Team team) {
        return !shouldLimit(team) ? Integer.MAX_VALUE : 10;
    }
    ...
    public void setBars() {
        ...
        addBar("maxPlace", (RemoteCoreStorageBuild entity) -> new Bar(
                        () -> Core.bundle.format("nh.bar.max-place", dsGroups.placedRemoteCore[entity.team.id].size, shouldLimit(entity.team)? maxPlaceNum(entity.team): "∞"),
                        () -> dsGroups.placedRemoteCore[entity.team.id].size < maxPlaceNum(entity.team) ? Pal.accent : Pal.redderDust,
                        () -> shouldLimit(entity.team)? (float) dsGroups.placedRemoteCore[entity.team.id].size / maxPlaceNum(entity.team): 1f
        ));
    ...
    public boolean canPlaceOn(Tile tile, Team team, int rotation) {
        return super.canPlaceOn(tile, team, rotation) && dsGroups.placedRemoteCore[team.id].size < maxPlaceNum(team);
    }
    ...
    public class RemoteCoreStorageBuild extends StorageBuild {
        ...
        public void onRemoved() {
            super.onRemoved();
            dsGroups.placedRemoteCore[team.id].remove(this);
        }

        @Override
        public void created() {
            Core.app.post(() -> dsGroups.placedRemoteCore[team.id].add(this));
        }

        @Override
        public void updateTile() {
            if (efficiency > 0 && core() != null && dsGroups.placedRemoteCore[team.id].size <= maxPlaceNum(team)) {
