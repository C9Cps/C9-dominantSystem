package dominantSystem.expand.block.production.drill;
...
import static dominantSystem.util.graphic.SpriteUtil.ATLAS_INDEX_4_4;
import static dominantSystem.util.graphic.SpriteUtil.orthogonalPos;

public class AdaptiveDrill extends Drill {

    public TextureRegion[] splits;

    public AdaptiveDrill(String name) {
        super(name);
    ...
    public void load() {
        super.load();
        if (size == 1) {
            splits = SpriteUtil.splitRegionArray(Core.atlas.find(name + "-autotile"), 32, 32, 1, ATLAS_INDEX_4_4);
        } else {
            splits = SpriteUtil.splitRegionArray(Core.atlas.find(name + "-autotile"), 32, 32, 1);
        }
    }

    public class AdaptiveDrillBuild extends DrillBuild {
        ...
        public void onProximityUpdate() {
            ...
            if (size == 1) {
                ...
                for (int i = 0; i < orthogonalPos.length; i++) {
                    ...
                    if (world.build(tileX() + p.x, tileY() + p.y) instanceof AdaptiveDrillBuild) {
                        drawIndex += 1 << i;
                    }
                }
            } else {
                for (int i = 0; i < Edges.getEdges(2).length; i++) {
                    ...
                    drawLink[i] = b instanceof AdaptiveDrillBuild;
                }
