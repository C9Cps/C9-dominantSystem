...
import dominantSystem.expand.block.inner.PlaceholderBlock;
...
public interface MultiBlock {
    ...
    default boolean checkLink(Tile tile, Team team, int size, int rotation) {
        // guard against uninitialized inner entities or invalid tile
        if (tile == null || team == null) return false;
        if (InnerBlock.linkEntity == null || InnerBlock.placeholderEntity == null) return false;
        for (int i = 0; i < linkBlockPos().size; i++) {
            Point2 p = linkBlockPos().get(i);
            int s = linkBlockSize().get(i);
            if (s <= 0 || s > InnerBlock.maxsize) return false;
            if (InnerBlock.linkEntity[s - 1] == null) return false;
    ...
    default void createPlaceholder(Tile tile, int size) {
        ...
        if (InnerBlock.placeholderEntity == null) return;

        for (int i = 0; i < linkBlockPos().size; i++) {
            Point2 p = linkBlockPos().get(i);
            int s = linkBlockSize().get(i);
            if (s <= 0 || s > InnerBlock.maxsize) continue;
            if (InnerBlock.placeholderEntity[s - 1] == null) continue;
    ...
    default Seq<Building> setLinkBuild(Building building, Block block, Tile tile, Team team, int size, int rotation) {
        ...
        if (tile == null || team == null) return out;
        for (int i = 0; i < linkBlockPos().size; i++) {
            Point2 p = linkBlockPos().get(i);
            int s = linkBlockSize().get(i);
            if (s <= 0 || s > InnerBlock.maxsize) continue;
            ...
            if (!block.outputsLiquid) {
                if (InnerBlock.linkEntity != null && InnerBlock.linkEntity[s - 1] != null) {
                    t.setBlock(InnerBlock.linkEntity[s - 1], team, 0);
                }
            } else {
                if (InnerBlock.linkEntityLiquid != null && InnerBlock.linkEntityLiquid[s - 1] != null) {
