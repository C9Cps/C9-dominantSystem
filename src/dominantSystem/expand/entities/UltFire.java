package dominantSystem.expand.entities;
...
public class UltFire extends Fire {
    ...
    public static final Effect remove = new Effect(70f, e -> {
        ...
        Drawf.light(e.x, e.y, 50f + Mathf.absin(5f, 5f), dsPal.thiriumAmmoFront, 0.6f * e.fout());
    });
    ...
    public void draw() {
        ...
        Draw.reset();
        Drawf.light(x, y, 50.0F + Mathf.absin(5.0F, 5.0F), dsPal.thiriumAmmoFront, 0.6F * Mathf.clamp(warmup / 20.0F));
    }
        ...
        if (!Vars.net.client()) {
            if (!(time >= lifetime) && tile != null && !Float.isNaN(lifetime)) {
                ...
                if ((damageTimer += Time.delta) >= 40.0F) {
                    ...
                    Damage.damageUnits(null, tile.worldx(), tile.worldy(), 8.0F, 10, (unit) -> !unit.isFlying() && !unit.isImmune(StatusEffects.burning), (unit) -> {
