package dominantSystem.expand.map;
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                if (tile.floor() != Blocks.air) {
                    floorTmp.add(new Stile(tile.floor(), x, y, null, (byte) 0));
                }
                if (tile.overlay() != Blocks.air) {
                    overlayTmp.add(new Stile(tile.overlay(), x, y, null, (byte) 0));
                }
                if (tile.block().isStatic() || tile.block() == Blocks.air) {
                    blockTmp.add(new Stile(tile.block(), x, y, null, (byte) 0));
                }
            }
        }
